...
let abilityShots=0,laserUntil=0,specialFx=[],multiplayerHit=null,multiplayerDraw=null;
...
function saveProgress(){
 try{
  rememberBrand();
  localStorage.setItem(SAVE_KEY,JSON.stringify({version:4,coins,brand,owned,profiles,questUnlocked,questLevel,turretEnabled}));
...
function loadProgress(){
 try{
  ...
  if(!data||![1,2,3,4].includes(data.version))return;
  ...
  if(integer(data.coins,Number.MAX_SAFE_INTEGER))coins=data.version<4&&data.coins===300000?0:data.coins;
...
function updateTurret(){
 if(!turretEnabled||document.body.classList.contains('in-multiplayer')||!['practice','quest'].includes(mode)||!active)return;
...
function drawTurret(){
 if(!turretEnabled||document.body.classList.contains('in-multiplayer')||mode==='forge')return;
